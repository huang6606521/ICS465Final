...
Template.My_Calendar_Page.onCreated(() => {
  let template = Template.instance();
  template.subscribe('Sessions');
  template.subscribe('Users');
});

Template.My_Calendar_Page.onRendered(() => {
  ...
  $('#sessions-calendar').fullCalendar({
    ...
    events(start, end, timezone, callback) {
      let data = Sessions.find({$or: [{guestsStuds: Meteor.user().profile.name}, {guestsPros: Meteor.user().profile.name}]} ).fetch().map((session) => {
...
Template.My_Calendar_Page.helpers({
  hasTutorial(){
